...
Pila pila_push(Pila pila, int dato){
  return(slist_agregar_inicio(pila,dato));
}

Pila pila_pop(Pila pila){
  if(pila_es_vacia(pila))return pila;
  SNodo *eli = pila;
  pila = pila->sig;
  free(eli);
  return pila;
...
//implementamos el reverso de una lista con pila

SList slist_reverso1(SList lista){
  if(slist_vacia(lista))return lista;
  Pila reversa = pila_crear();
  SNodo *temp = lista;
  for(;temp != NULL ; temp = temp->sig){
    reversa = pila_push(reversa,temp->dato);
  }
  return reversa;
}
